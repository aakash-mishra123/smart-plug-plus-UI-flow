...
function getVariantStyles(tabVariant: TabsListVariant) {
  switch (tabVariant) {
    case "line":
      return cx(
        // base
        "-mb-px items-center justify-center whitespace-nowrap border-b-4 border-transparent px-3 pb-2 text-sm font-bold transition-all",
        // text color
        ...
        // selected
        "data-[state=active]:border-blue-700 data-[state=active]:text-[#205fcb]",
        "data-[state=active]:dark:border-blue-700 data-[state=active]:dark:text-[#205fcb]",
        // disabled
...
>(({ className, children, onTabClick, ...props }, forwardedRef) => {
  ...
  return (
    <TabsPrimitives.Trigger
      ...
      onClick={(event) => {
        props.onClick?.(event);
        if (onTabClick) {
          onTabClick(props.value as string);
        }
      }}
      {...props}
    >
